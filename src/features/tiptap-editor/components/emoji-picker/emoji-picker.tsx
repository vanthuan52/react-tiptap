...
const VirtualizedRow = ({
  ...
  return (
    <div style={style} className="rte-emoji-picker__row">
      {emojis.map((emoji, i) => (
        <button
          key={`${emoji.name}-${i}`}
          type="button"
  ...
  return (
    <div className="rte-emoji-picker">
      <SearchInput
        autoFocus={false}
        placeholder="Search"
        className="rte-emoji-picker__search"
        value={query}
        onChange={handleSearch}
      />

      <div className="rte-emoji-picker__content">
        {isEmpty ? (
          <div className="rte-emoji-picker__empty">
            <p>No emojis found</p>
          </div>
        ) : (
          ...
          >
            {headers.map((header) => (
              ...
              >
                <div className="rte-emoji-picker__title">{header.title}</div>
              </div>
            ))}
          </List>
        )}
      ...
      <div className="rte-emoji-picker__preview">
        {hoveredEmoji ? (
          <>
            <span className="rte-emoji-picker__preview-emoji">{hoveredEmoji.emoji}</span>
            <span className="rte-emoji-picker__preview-text">{hoveredEmoji.name}</span>
          </>
        ) : (
          <span className="rte-emoji-picker__preview-text">Select an emoji...</span>
        )}
      </div>
    </div>
  );
